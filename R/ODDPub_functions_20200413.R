...
.map_keywords <- function(publ_sentences)
{
  ...
  publ_keywords <- sentence_search_keywords %>%
    purrr::map(.search_keyword_cat, publ_sentences, keyword_list)
...
.search_keyword_cat <- function(keyword, sentences, keyword_list)
{
  detection_col <- sentences %>%
    purrr::map_lgl(stringr::str_detect, pattern = keyword_list[[keyword]])
...
.keyword_search_full <- function(PDF_text_sentences)
{
  ...
  open_data_categories <- purrr::map(PDF_text_sentences, .map_keywords)

  #combine columns for the different open data keywords
  keyword_results_combined <- open_data_categories %>%
    purrr::map(mutate, com_specific_db = field_specific_db & accession_nr & available & !not_available & !was_available) %>%
    purrr::map(mutate, com_general_db = repositories & available & !not_available & !was_available) %>%
    purrr::map(mutate, com_github_data = data & github & available & !not_available & !was_available) %>%
    purrr::map(mutate, com_code = source_code & available & !not_available & !was_available & !upon_request) %>%
    purrr::map(mutate, com_suppl_code = supplement & source_code) %>%
    purrr::map(select, publ_sentences, com_specific_db, com_general_db, com_github_data, dataset, com_code, com_suppl_code)
...
.keyword_search_tokenized <- function(keyword_results_combined)
{
  ...
  keyword_results_tokenized <- keyword_results_combined %>%
    purrr::map(select, -publ_sentences) %>%
    purrr::map(apply, MARGIN = 2, FUN = any)
...
.keyword_search_near_wd <- function(PDF_text_sentences, extract_text = FALSE)
{
  ...
  PDF_text_full <- PDF_text_sentences %>% purrr::map(paste, collapse = " ")
...

{
  ...
  #identifies the text fragments in which the Open Data keywords were detected
  open_data_sentences <- purrr::map(keyword_results, .text_fragments)
